...
/// step shapes: `cargo test --locked --release` and
/// `cargo test --locked --release -- --ignored`, the default selection
/// and the lzop oracle selection, both in the shipped profile and
/// neither carrying `EXPECT_OVERFLOW_CHECKS`. (In the file itself each
/// of those commands sits inside a `run: |` block that tees the output
/// and counts the tests that ran; the counting is beside the point
/// here, and the commands are copied rather than invented.)
///
/// THIS REPOSITORY'S `release.yml` CARRIES NO DEBUG RUN AT ALL, which
/// is the inverse of the sibling this guard was copied from. Both of
/// its `cargo test` lines name `--release`, so the parser finds nothing
/// in it that could even be mistaken for the run `ci.yml` is required
/// to have. The first assertion below pins that, and it is worth
/// pinning: if someone later drops `--release` from a release step to
/// chase a failure, `release.yml` acquires a debug run, and the guard
/// must still refuse to count it.
///
/// The scenario worth pinning after that is the near miss: even a
/// hypothetical debug run in `release.yml` that DID set the handshake
/// would not make `ci.yml`'s own absence of one acceptable, because
/// `release.yml` triggers too late to gate a merge. Both shapes are
/// asserted below.
///
...
";
    // Both name `--release`, so neither is a debug run; and the second
    // would not be one even without it, because `-- --ignored` runs
    // ONLY `#[ignore]` tests -- in this crate the lzop oracle suite --
    // and the probe in src/lib.rs is an ordinary library unit test that
    // such a selection filters out. See `omits_the_library_unit_tests`.
    assert!(
        scan_steps(release_yml_as_it_is, false, runs_with_overflow_checks).is_empty(),
        "release.yml runs only the shipped profile, so it contains no debug \
         run for the guard to be tempted by -- and the guard would not count \
         one anyway, because it never opens that file"
    );
    assert!(
        scan_steps(
        ...
        .is_empty(),
        "release.yml carries no handshake, and is not asked to: overflow \
         checks are off in release deliberately, and the handshake would arm \
         an assertion that fails every green tag build"
    );
...
jobs:
  test:
    steps:
      - run: EXPECT_OVERFLOW_CHECKS=1 cargo test --locked
";
    assert_eq!(
        scan_steps(
        ...
        vec!["EXPECT_OVERFLOW_CHECKS=1 cargo test --locked".to_string()],
