...
mod overflow_checks {
    ...
    ///
    /// An LZO1X decoder is a loop over lengths and back-references read
    /// out of untrusted input, and the arithmetic in that loop IS the
    /// parser: match lengths accumulated 255 at a time across a run of
    /// zero bytes, distances subtracted from an output position, literal
    /// runs added to it. A computation that wraps there does not stay an
    /// abstract concern -- it is precisely how a decompressor turns a
    /// malformed stream into an out-of-bounds index. With the checks off
    /// the wrapped value either lands back inside the buffer (wrong
    /// output, no failure) or trips a later bounds check that reports a
    /// generic error naming nothing. Either way the suite sees a pass or
    /// a plausible error, never the panic that names the line.
    ///
